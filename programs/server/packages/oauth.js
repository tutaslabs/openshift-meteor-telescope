...
OAuth._redirectUri = function (serviceName, config, params, absoluteUrlOptions) {                 // 3
  ...
  var query = config.loginStyle ? null : "close";                                                 // 9
                                                                                                  // 10
  // Clone because we're going to mutate 'params'. The 'cordova' and                              // 11
  // 'android' parameters are only used for picking the host of the                               // 12
  // redirect URL, and not actually included in the redirect URL itself.                          // 13
  var isCordova = false;                                                                          // 14
  var isAndroid = false;                                                                          // 15
  if (params) {                                                                                   // 16
    params = _.clone(params);                                                                     // 17
    isCordova = params.cordova;                                                                   // 18
    isAndroid = params.android;                                                                   // 19
    delete params.cordova;                                                                        // 20
    delete params.android;                                                                        // 21
    if (_.isEmpty(params)) {                                                                      // 22
      params = undefined;                                                                         // 23
    }                                                                                             // 24
  }                                                                                               // 25
                                                                                                  // 26
  if (Meteor.isServer && isCordova) {                                                             // 27
    var rootUrl = process.env.MOBILE_ROOT_URL ||                                                  // 28
          __meteor_runtime_config__.ROOT_URL;                                                     // 29
                                                                                                  // 30
    if (isAndroid) {                                                                              // 31
      // Match the replace that we do in cordova boilerplate                                      // 32
      // (boilerplate-generator package).                                                         // 33
      // XXX Maybe we should put this in a separate package or something                          // 34
      // that is used here and by boilerplate-generator? Or maybe                                 // 35
      // `Meteor.absoluteUrl` should know how to do this?                                         // 36
      var url = Npm.require("url");                                                               // 37
      var parsedRootUrl = url.parse(rootUrl);                                                     // 38
      if (parsedRootUrl.hostname === "localhost") {                                               // 39
        parsedRootUrl.hostname = "10.0.2.2";                                                      // 40
        delete parsedRootUrl.host;                                                                // 41
      }                                                                                           // 42
      rootUrl = url.format(parsedRootUrl);                                                        // 43
    }                                                                                             // 44
                                                                                                  // 45
    absoluteUrlOptions = _.extend({}, absoluteUrlOptions, {                                       // 46
      // For Cordova clients, redirect to the special Cordova root url                            // 47
      // (likely a local IP in development mode).                                                 // 48
      rootUrl: rootUrl                                                                            // 49
    });                                                                                           // 50
  }                                                                                               // 51
                                                                                                  // 52
  return URL._constructUrl(                                                                       // 53
    Meteor.absoluteUrl('_oauth/' + serviceName, absoluteUrlOptions),                              // 54
    query,                                                                                        // 55
    params);                                                                                      // 56
};                                                                                                // 57
                                                                                                  // 58
