...
Renderer.prototype.link = function(href, title, text) {                        // 862
  if (this.options.sanitize) {                                                 // 863
    ...
    if (prot.indexOf('javascript:') === 0) {                                   // 871
      return '';                                                               // 872
    }                                                                          // 873
  }                                                                            // 874
...
function marked(src, opt, callback) {                                          // 1135
  if (callback || typeof opt === 'function') {                                 // 1136
    ...
    pending = tokens.length;                                                   // 1155
                                                                               // 1156
    var done = function() {                                                    // 1157
      var out, err;                                                            // 1158
                                                                               // 1159
      try {                                                                    // 1160
        out = Parser.parse(tokens, opt);                                       // 1161
      } catch (e) {                                                            // 1162
        err = e;                                                               // 1163
      }                                                                        // 1164
                                                                               // 1165
      opt.highlight = highlight;                                               // 1166
                                                                               // 1167
      return err                                                               // 1168
        ? callback(err)                                                        // 1169
        : callback(null, out);                                                 // 1170
    };                                                                         // 1171
                                                                               // 1172
    if (!highlight || highlight.length < 3) {                                  // 1173
      return done();                                                           // 1174
    }                                                                          // 1175
                                                                               // 1176
    delete opt.highlight;                                                      // 1177
                                                                               // 1178
    if (!pending) return done();                                               // 1179
                                                                               // 1180
    for (; i < tokens.length; i++) {                                           // 1181
      (function(token) {                                                       // 1182
        if (token.type !== 'code') {                                           // 1183
          return --pending || done();                                          // 1184
        }                                                                      // 1185
        return highlight(token.text, token.lang, function(err, code) {         // 1186
          if (code == null || code === token.text) {                           // 1187
            return --pending || done();                                        // 1188
          }                                                                    // 1189
          token.text = code;                                                   // 1190
          token.escaped = true;                                                // 1191
          --pending || done();                                                 // 1192
        });                                                                    // 1193
      })(tokens[i]);                                                           // 1194
    }                                                                          // 1195
  ...
  try {                                                                        // 1199
    if (opt) opt = merge({}, marked.defaults, opt);                            // 1200
    return Parser.parse(Lexer.lex(src, opt), opt);                             // 1201
  } catch (e) {                                                                // 1202
    e.message += '\nPlease report this to https://github.com/chjj/marked.';    // 1203
    if ((opt || marked.defaults).silent) {                                     // 1204
      return '<p>An error occured:</p><pre>'                                   // 1205
        + escape(e.message + '', true)                                         // 1206
        + '</pre>';                                                            // 1207
    }                                                                          // 1208
    throw e;                                                                   // 1209
  }                                                                            // 1210
 ...
 */                                                                            // 1215
                                                                               // 1216
marked.options =                                                               // 1217
marked.setOptions = function(opt) {                                            // 1218
  merge(marked.defaults, opt);                                                 // 1219
  return marked;                                                               // 1220
};                                                                             // 1221
                                                                               // 1222
marked.defaults = {                                                            // 1223
  gfm: true,                                                                   // 1224
  tables: true,                                                                // 1225
  breaks: false,                                                               // 1226
  pedantic: false,                                                             // 1227
  sanitize: false,                                                             // 1228
  smartLists: false,                                                           // 1229
  silent: false,                                                               // 1230
  highlight: null,                                                             // 1231
  langPrefix: 'lang-',                                                         // 1232
  smartypants: false,                                                          // 1233
  headerPrefix: '',                                                            // 1234
  renderer: new Renderer,                                                      // 1235
  xhtml: false                                                                 // 1236
};                                                                             // 1237
                                                                               // 1238
/**                                                                            // 1239
 ...
 */                                                                            // 1241
                                                                               // 1242
marked.Parser = Parser;                                                        // 1243
marked.parser = Parser.parse;                                                  // 1244
                                                                               // 1245
marked.Renderer = Renderer;                                                    // 1246
                                                                               // 1247
marked.Lexer = Lexer;                                                          // 1248
marked.lexer = Lexer.lex;                                                      // 1249
                                                                               // 1250
marked.InlineLexer = InlineLexer;                                              // 1251
marked.inlineLexer = InlineLexer.output;                                       // 1252
                                                                               // 1253
marked.parse = marked;                                                         // 1254
                                                                               // 1255
if (typeof exports === 'object') {                                             // 1256
  module.exports = marked;                                                     // 1257
} else if (typeof define === 'function' && define.amd) {                       // 1258
  define(function() { return marked; });                                       // 1259
} else {                                                                       // 1260
  this.marked = marked;                                                        // 1261
}                                                                              // 1262
                                                                               // 1263
}).call(function() {                                                           // 1264
  return this || (typeof window !== 'undefined' ? window : global);            // 1265
}());                                                                          // 1266
                                                                               // 1267
/////////////////////////////////////////////////////////////////////////////////
