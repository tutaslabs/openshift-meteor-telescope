...
CollectionHooks = {                                                                                                  // 40
  defaults: {                                                                                                        // 41
    ...
    all: { insert: {}, update: {}, remove: {}, find: {}, findOne: {}, all: {}}                                       // 44
                                                                                                                     ...
                                                                                                                     // 47
CollectionHooks.extendCollectionInstance = function (self) {                                                         // 48
  ...
  _.each(advices, function (advice, method) {                                                                        // 86
    ...
    self.direct[method] = function () {                                                                              // 91
      ...
      return directOp(function () {                                                                                  // 93
        return collection[method].apply(collection, args);                                                           // 94
      });                                                                                                            // 95
    };                                                                                                               // 96
                                                                                                                     // 97
    collection[method] = function () {                                                                               // 98
      return advice.call(this,                                                                                       // 99
        getUserId(),                                                                                                 // 100
        _super,                                                                                                      // 101
        self,                                                                                                        // 102
        self._hookAspects[method] || {},                                                                             // 103
        function (doc) {                                                                                             // 104
          return  _.isFunction(self._transform)                                                                      // 105
                  ? function (d) { return self._transform(d || doc); }                                               // 106
                  : function (d) { return d || doc; };                                                               // 107
        },                                                                                                           // 108
        _.toArray(arguments),                                                                                        // 109
        directEnv.get() === true                                                                                     // 110
      );                                                                                                             // 111
    };                                                                                                               // 112
  });                                                                                                                // 113
};                                                                                                                   // 114
                                                                                                                     // 115
CollectionHooks.defineAdvice = function (method, advice) {                                                           // 116
  advices[method] = advice;                                                                                          // 117
};                                                                                                                   // 118
                                                                                                                     // 119
CollectionHooks.initOptions = function (options, pointcut, method) {                                                 // 120
  return CollectionHooks.extendOptions(CollectionHooks.defaults, options, pointcut, method);                         // 121
};                                                                                                                   // 122
                                                                                                                     // 123
CollectionHooks.extendOptions = function (source, options, pointcut, method) {                                       // 124
  options = _.extend(options || {}, source.all.all);                                                                 // 125
  options = _.extend(options, source[pointcut].all);                                                                 // 126
  options = _.extend(options, source.all[method]);                                                                   // 127
  options = _.extend(options, source[pointcut][method]);                                                             // 128
  return options;                                                                                                    // 129
};                                                                                                                   // 130
                                                                                                                     // 131
CollectionHooks.getDocs = function (collection, selector, options) {                                                 // 132
  var self = this;                                                                                                   // 133
                                                                                                                     // 134
  var findOptions = {transform: null, reactive: false}; // added reactive: false                                     // 135
                                                                                                                     // 136
  /*                                                                                                                 // 137
  // No "fetch" support at this time.                                                                                // 138
  if (!self._validators.fetchAllFields) {                                                                            // 139
    findOptions.fields = {};                                                                                         // 140
    _.each(self._validators.fetch, function(fieldName) {                                                             // 141
      findOptions.fields[fieldName] = 1;                                                                             // 142
    });                                                                                                              // 143
  }                                                                                                                  // 144
  */                                                                                                                 // 145
                                                                                                                     // 146
  // Bit of a magic condition here... only "update" passes options, so this is                                       // 147
  // only relevant to when update calls getDocs:                                                                     // 148
  if (options) {                                                                                                     // 149
    // This was added because in our case, we are potentially iterating over                                         // 150
    // multiple docs. If multi isn't enabled, force a limit (almost like                                             // 151
    // findOne), as the default for update without multi enabled is to affect                                        // 152
    // only the first matched document:                                                                              // 153
    if (!options.multi) {                                                                                            // 154
      findOptions.limit = 1;                                                                                         // 155
    }                                                                                                                // 156
  }                                                                                                                  // 157
                                                                                                                     // 158
  // Unlike validators, we iterate over multiple docs, so use                                                        // 159
  // find instead of findOne:                                                                                        // 160
  return collection.find(selector, findOptions);                                                                     // 161
};                                                                                                                   // 162
                                                                                                                     // 163
CollectionHooks.reassignPrototype = function (instance, constr) {                                                    // 164
  var hasSetPrototypeOf = typeof Object.setPrototypeOf === "function";                                               // 165
                                                                                                                     // 166
  if (!constr) constr = typeof Mongo !== "undefined" ? Mongo.Collection : Meteor.Collection;                         // 167
                                                                                                                     // 168
  // __proto__ is not available in < IE11                                                                            // 169
  // Note: Assigning a prototype dynamically has performance implications                                            // 170
  if (hasSetPrototypeOf) {                                                                                           // 171
    Object.setPrototypeOf(instance, constr.prototype);                                                               // 172
  } else if (instance.__proto__) {                                                                                   // 173
    instance.__proto__ = constr.prototype;                                                                           // 174
  }                                                                                                                  // 175
};                                                                                                                   // 176
                                                                                                                     // 177
CollectionHooks.wrapCollection = function (ns, as) {                                                                 // 178
  if (!as._CollectionConstructor) as._CollectionConstructor = as.Collection;                                         // 179
  if (!as._CollectionPrototype) as._CollectionPrototype = new as.Collection(null);                                   // 180
                                                                                                                     // 181
  var constructor = as._CollectionConstructor;                                                                       // 182
  var proto = as._CollectionPrototype;                                                                               // 183
                                                                                                                     // 184
  ns.Collection = function () {                                                                                      // 185
    var ret = constructor.apply(this, arguments);                                                                    // 186
    CollectionHooks.extendCollectionInstance(this);                                                                  // 187
    return ret;                                                                                                      // 188
  };                                                                                                                 // 189
                                                                                                                     // 190
  ns.Collection.prototype = proto;                                                                                   // 191
                                                                                                                     // 192
  for (var prop in constructor) {                                                                                    // 193
    if (constructor.hasOwnProperty(prop)) {                                                                          // 194
      ns.Collection[prop] = constructor[prop];                                                                       // 195
    }                                                                                                                // 196
  }                                                                                                                  // 197
};                                                                                                                   // 198
                                                                                                                     // 199
if (typeof Mongo !== "undefined") {                                                                                  // 200
  CollectionHooks.wrapCollection(Meteor, Mongo);                                                                     // 201
  CollectionHooks.wrapCollection(Mongo, Mongo);                                                                      // 202
} else {                                                                                                             // 203
  CollectionHooks.wrapCollection(Meteor, Meteor);                                                                    // 204
}                                                                                                                    // 205
                                                                                                                     // 206
if (Meteor.isServer) {                                                                                               // 207
  var _publish = Meteor.publish;                                                                                     // 208
  Meteor.publish = function (name, func) {                                                                           // 209
    return _publish.call(this, name, function () {                                                                   // 210
      // This function is called repeatedly in publications                                                          // 211
      var ctx = this, args = arguments;                                                                              // 212
      return publishUserId.withValue(ctx && ctx.userId, function () {                                                // 213
        return func.apply(ctx, args);                                                                                // 214
      });                                                                                                            // 215
    });                                                                                                              // 216
  };                                                                                                                 // 217
                                                                                                                     // 218
  // Make the above available for packages with hooks that want to determine                                         // 219
  // whether they are running inside a publish function or not.                                                      // 220
  CollectionHooks.isWithinPublish = function () {                                                                    // 221
    return publishUserId.get() !== undefined;                                                                        // 222
  };                                                                                                                 // 223
}                                                                                                                    // 224
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
